...
package engine
{
	...
	public class FrameSprite extends Sprite implements IAnimatable
	{
		...
		private var m_tileSize:uint;
		
		public function FrameSprite(_tileSize:uint, _fps:Number = 24)
		{
			super();
		...
		public function addFrame(_label:String = ""):void
		{
			...
			if (_label !== "") frame.name = _label;
			
			m_totalFrames = m_frames.push(frame);
			
			if (m_totalFrames == 1) addChild(frame);
		}
		...
		public function removeFrame(_val:*):void
		{
			...
			if (frameNum == m_currentFrame) prevFrame();
		...
		{
			if (!isPlaying || m_totalFrames == 0) return;
		...
		private function getFrame(_val:*):uint
		{
			...
			if (isNaN(_val))
			{
				...
				while(i--)
				{
					if (m_frames[i].name == _val) return i + 1;
				}
			}
		...
		public function get isPlaying():Boolean { return m_isPlaying; }
		public function get currentFrameLabel():String { return m_frames[m_currentFrame - 1].name; }
		public function get totalFrames():int { return m_totalFrames; }
		public function get loop():Boolean { return m_loop; }
		public function set loop(value:Boolean):void { m_loop = value; }
